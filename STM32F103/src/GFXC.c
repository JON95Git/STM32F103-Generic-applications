...
int8_t PChar(uint16_t Ch, TextParamStruct *T)
{
	...
	if (X>XPix-(Size+1)*5 || Y>YPix-(Size+1)*8 || Ch < 0x20)
		return -1;
	...
	case 0:
		...
		for (YCnt = 0; YCnt<8; YCnt++)
		{
			for(XCnt = 0; XCnt<5+LetterSpace; XCnt++)
			{
				...
				if (XCnt<5 && (Chars[Ch+XCnt] & (1<<YCnt)))
					    SW(T->TxtCol, Dat);
	...
	case 1:
		...
		for(YCnt = 0; YCnt<16; YCnt++){
			for(XCnt = 0; XCnt<10+LetterSpace; XCnt++){
				...
				if (XCnt<10 && (Chars[Ch+(XCnt>>1)] & (1<<(YCnt>>1))))
						SW(T->TxtCol, Dat);
	...
	case 2:
		...
		for (YCnt = 0; YCnt<24; YCnt++)
		{
			for(XCnt = 0; XCnt<15+LetterSpace; XCnt++)
			{
				...
				if (XCnt<15 && (Chars[Ch+((86*XCnt)>>8)] & (1<<((86*YCnt)>>8))))
						SW(T->TxtCol, Dat);
...
int8_t PStr(const char* Str, TextParamStruct *T)
{
	...
	{
	case 0:
		if (X>XPix - StrL*(5+LetterSpace) || Y>YPix-8)
				return -1;
		...
		break;
	case 1:
		if (X>XPix - StrL*(10+LetterSpace) || Y>YPix-16)
				return -1;
		...
		break;
	case 2:
		if (X>XPix - StrL*(15+LetterSpace) || Y>YPix-24)
			return -1;
...
uint8_t Ellipse(uint8_t XS, uint8_t YS, uint8_t XR, uint8_t YR, Colours8 Col)
{
	...
	if ((XS + XR) < 0 || (XS + XR) > XPix || (YS + YR) < 0 || (YS + YR) > YPix)
		return 1;
...
uint8_t Semicircle(uint8_t XS, uint8_t YS, uint8_t R, uint8_t Rot, Colours8 Col)
{
	...
	case 0:
	case 2:
		LineP(XS-(R-1), YS, XS+(R), YS, Col);
		break;
	case 1:
	case 3:
		LineP(XS, YS-(R-1), XS, YS+R, Col);
		break;
	}
	return 0;
}
